...
; Tests for universe_crypto_sha1_*: FIPS 180-4 known-answer vectors,
...
@d.empty = private unnamed_addr constant [20 x i8] c"\da\39\a3\ee\5e\6b\4b\0d\32\55\bf\ef\95\60\18\90\af\d8\07\09"
@d.abc   = private unnamed_addr constant [20 x i8] c"\a9\99\3e\36\47\06\81\6a\ba\3e\25\71\78\50\c2\6c\9c\d0\d8\9d"
@d.nist2 = private unnamed_addr constant [20 x i8] c"\84\98\3e\44\1c\3b\d2\6e\ba\ae\4a\a1\f9\51\29\e5\e5\46\70\f1"

@g.src = internal global [70000 x i8] zeroinitializer, align 16
@g.ctx = internal global [96 x i8] zeroinitializer, align 8
@g.out = internal global [20 x i8] zeroinitializer, align 8
@g.out2 = internal global [20 x i8] zeroinitializer, align 8
...
@m.empty = private unnamed_addr constant [14 x i8] c"sha1(empty)\00\00\00"
@m.abc   = private unnamed_addr constant [10 x i8] c"sha1(abc)\00"
@m.nist2 = private unnamed_addr constant [12 x i8] c"sha1(nist2)\00"
...
@fmt.bench = private unnamed_addr constant [32 x i8] c"bench sha1 64KiB: %.2f MB/s\0A\00\00\00\00"
@sha1.samp = internal global [16 x double] zeroinitializer, align 8
@lbl.sha1 = private unnamed_addr constant [22 x i8] c"sha1 64KiB (per byte)\00"

define i32 @main(i32 %argc, ptr %argv) {
entry:
  call void @universe_crypto_sha1_hash(ptr @g.src, i64 0, ptr @g.out)
  %ce = call i32 @memcmp(ptr @g.out, ptr @d.empty, i64 20)
  %ce.ok = icmp eq i32 %ce, 0
  call void @ut_check(i1 %ce.ok, ptr @m.empty)

  call void @universe_crypto_sha1_hash(ptr @k.abc, i64 3, ptr @g.out)
  %ca = call i32 @memcmp(ptr @g.out, ptr @d.abc, i64 20)
  %ca.ok = icmp eq i32 %ca, 0
  call void @ut_check(i1 %ca.ok, ptr @m.abc)

  call void @universe_crypto_sha1_hash(ptr @k.nist2, i64 56, ptr @g.out)
  %cn = call i32 @memcmp(ptr @g.out, ptr @d.nist2, i64 20)
  %cn.ok = icmp eq i32 %cn, 0
  call void @ut_check(i1 %cn.ok, ptr @m.nist2)

  ; ---- streaming odd chunks over 56-byte msg == one-shot ----
  call void @universe_crypto_sha1_init(ptr @g.ctx)
  br label %sc.head
...
sc.body:
  ...
  call void @universe_crypto_sha1_update(ptr @g.ctx, ptr %dp, i64 %csz)
  %si.n = add nuw i64 %si, %csz
  br label %sc.head

sc.fin:
  call void @universe_crypto_sha1_final(ptr @g.ctx, ptr @g.out)
  call void @universe_crypto_sha1_hash(ptr @k.nist2, i64 56, ptr @g.out2)
  %sccmp = call i32 @memcmp(ptr @g.out, ptr @g.out2, i64 20)
...
rt.head:
  ...
  call void @universe_crypto_sha1_hash(ptr @g.src, i64 %len, ptr @g.out2)
  call void @universe_crypto_sha1_init(ptr @g.ctx)
  br label %rs.head
...
rs.body:
  ...
  call void @universe_crypto_sha1_update(ptr @g.ctx, ptr %rdp, i64 %rcsz)
  %ri.n = add nuw i64 %ri, %rcsz
  br label %rs.head

rs.fin:
  call void @universe_crypto_sha1_final(ptr @g.ctx, ptr @g.out)
  %rcmp = call i32 @memcmp(ptr @g.out, ptr @g.out2, i64 20)
...
be.head:
  ...
  call void @universe_crypto_sha1_hash(ptr @g.src, i64 65536, ptr @g.out)
...
bstore:
  ...
  %bsp = getelementptr inbounds [16 x double], ptr @sha1.samp, i64 0, i64 %bslot
...
brep.report:
  call void @ut_report_dist(ptr @sha1.samp, i64 16, i64 131072000, ptr @lbl.sha1)
  br label %fin
